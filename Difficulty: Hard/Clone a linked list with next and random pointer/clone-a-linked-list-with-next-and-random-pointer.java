...
class Solution {
    ...
    Node copyList(Node head) {
        ...
        1. create a new linkedlist using while loop
            in this linkedlist the next pointer of original LL 
            points to the same node in duplicate LL
            Then that duplicate node points to the original next node 
            This figure looks like zigzag pattern with a set of duplicate elements
            
        2. another while loop to set the connections for random pointers
            as similar to the random connections in the original LL
            Set the random connections for the duplicate LL too
        
        3. Another while loop to remove the connections bw the 2 LL's
            makes two seperate LL's
        */
        
        Node cur= head;
        //inserting new nodes in bw
        while(cur!=null){
            Node temp= cur.next;
        ...
        cur= head;
        //creating the connections as per random pointers
        while(cur!=null){
            if(cur.next!=null){
                cur.next.random = (cur.random!=null) ? cur.random.next : null;
                //cur.random points to the random pointer
                //cur.random.next points to the same node but the duplpicate one
            }
            cur= cur.next.next;
        }
